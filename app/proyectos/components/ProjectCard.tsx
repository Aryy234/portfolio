...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  const handleVideoHover = () => {
    if (videoError || !project.videoUrl) return
      ...
      <Link href={projectDetailUrl}>
        ...
        >
          <Image
            ...
            alt={project.titulo}
            fill
            className={`object-cover transition-opacity duration-500 ${activeVideo && videoLoaded && !videoError ? "opacity-0" : "opacity-100"}`}
          />
          ...
          {project.videoUrl && !videoError && (
            <video
              ...
              className={`absolute inset-0 w-full h-full object-cover transition-all duration-500 ${
                activeVideo && videoLoaded ? "opacity-100 scale-105" : "opacity-0 scale-95"
              }`}
              muted
              playsInline
              preload="metadata"
          ...
          {project.videoUrl && !videoError && (
            <div className="absolute bottom-2 left-2 bg-black bg-opacity-50 text-white text-xs py-1 px-2 rounded-lg flex items-center gap-1">
              <Play size={12} />
